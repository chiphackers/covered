...
int main() {
  ...
  result = expression_create( right, left, EXP_OP_DIVIDE, 3, 0 );

  fprintf( stderr, "Running diagnostic: divide\n" );
  ...
  for( i=0; i<16; i++ ) {
    ...
    for( j=1; j<16; j++ ) {
      ...
      if( vector_to_int( result->value ) != (i / j) ) {
        print_output( "Division result does not match expected value", FATAL );
