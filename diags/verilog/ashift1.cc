...
#include "Vashift1.h"                 // From Verilating "ashift1.v"
#ifdef COVERED_INLINED
#include "Vashift1_main.h"
#endif
#include <SpTraceVcdC.h>           // Trace file format header (from SystemPerl)

Vashift1 *top;                        // Instantiation of module
...
int main() {
  ...
  top = new Vashift1;                 // Create instance

  Verilated::traceEverOn( true );  // Verilator must compute traced signals
  SpTraceVcdCFile* tfp = new SpTraceVcdCFile;
  top->trace( tfp, 99 );           // Trace 99 levels of hierarchy
  tfp->open( "ashift1.vcd" );         // Open the dump file

#ifdef COVERED_INLINED
  covered_initialize( top, "../ashift1.cdd" );
#endif
