...
#include "Vashift2.1.h"                 // From Verilating "ashift2.1.v"
#include <SpTraceVcdC.h>           // Trace file format header (from SystemPerl)

Vashift2.1 *top;                        // Instantiation of module
...
int main() {
  ...
  top = new Vashift2.1;                 // Create instance

  Verilated::traceEverOn( true );  // Verilator must compute traced signals
  SpTraceVcdCFile* tfp = new SpTraceVcdCFile;
  top->trace( tfp, 99 );           // Trace 99 levels of hierarchy
  tfp->open( "ashift2.1.vcd" );         // Open the dump file
