...
if( $SIMULATOR eq "IV" ) {
  ...
  system( "cver +define+DUMP exclude10.3.v" ) && die;
...
&runCommand( "echo This logic is not needed > exclude10.3.excl" );

# Perform exclusion
&runExcludeCommand( "-m E03 exclude10.3.cdd < exclude10.3.excl" );

# Remove temporary exclusion reason file
system( "rm -f exclude10.3.excl" ) && die;
