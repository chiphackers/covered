...
\item[Parameters:]
\begin{description}
\item[{\em name}]Name of signal to set value to. \item[{\em symbol}]Symbol value of signal used in VCD dumpfile.\end{description}
\end{Desc}
...
\end{Desc}
Removes specified statement expression from current module expression list and deallocates both the expression and statement from heap memory. Called when a statement structure is found to contain a statement that is not supported by Covered. \index{db.h@{db.h}!db_set_symbol_char@{db\_\-set\_\-symbol\_\-char}}
