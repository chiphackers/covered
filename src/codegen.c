...
char* codegen_gen_expr( expression* expr, int line ) {
  ...
  bool  both;            /* Specifies if both expressions or just right expression should be used     */
  ...
  if( (expr != NULL) && ((line == -1) || (expr->line == line)) ) {
    ...
    */
    ...
    if( SUPPL_OP( expr->suppl ) == EXP_OP_LAST ) {
      ...
      snprintf( my_code, (strlen( expr->sig->name ) + 13), "%s[%d]", expr->sig->name, right_code );

    } else if( SUPPL_OP( expr->suppl ) == EXP_OP_MBIT_SEL ) {

      assert( expr->sig != NULL );
      my_code = (char*)malloc_safe( strlen( expr->sig->name ) + 23 );
      snprintf( my_code, (strlen( expr->sig->name ) + 13), "%s[%d:%d]", expr->sig->name, left_code, right_code );
