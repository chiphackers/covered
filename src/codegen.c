...
static void codegen_gen_expr1(
  ...
  /*@out@*/ unsigned int* code_depth,  /*!< Pointer to number of strings contained in code array */
            ...
            unsigned int  exp_depth    /*!< Specifies the current depth of the given expression */
  ...
  if( expr != NULL ) {
    ...
    /* Recalculate the current expression depth */
    exp_depth += ((expr->op != parent_op) ? 1 : 0);

    /* If the current expression will be calculated intermediately, just output the intermediate expression name */
    if( inline_exp && generator_expr_name_needed( expr, exp_depth ) ) {
    ...
    /* Otherwise, calculate the value as is deemed necessary */
      ...
      if( (expr->op != EXP_OP_SLIST) && (expr->op != EXP_OP_ALWAYS_COMB) && (expr->op != EXP_OP_ALWAYS_LATCH) ) {
        ...
        codegen_gen_expr1( expr->left,  expr->op, &left_code,  &left_code_depth,  funit, inline_exp, exp_depth );
        codegen_gen_expr1( expr->right, expr->op, &right_code, &right_code_depth, funit, inline_exp, exp_depth );
...
/*
 ...
 Revision 1.97  2008/06/27 14:02:00  phase1geo
